...
class CollisionDetector : NSObject, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact:SKPhysicsContact){
        if (contact.bodyA.node != nil && contact.bodyB.node != nil)  {
            ...
            else if ( name1 == PlayerSpawn.SpriteName() && name2 == Enemy.SpriteName())
            {
                playerHitHandler?(node1,node2,contact.contactPoint)
            }
            else if ( name2 == Enemy.SpriteName() && name1 == PlayerSpawn.SpriteName())
            {
                playerHitHandler?(node2,node1,contact.contactPoint)
            }
            else if ( name1 == PlayerSpawn.SpriteName() && name2 == EnemyShot.SpriteName())
            {
                playerHitHandler?(node1,node2,contact.contactPoint)
            }
            else if ( name2 == EnemyShot.SpriteName() && name1 == PlayerSpawn.SpriteName())
            {
                playerHitHandler?(node2,node1,contact.contactPoint)
            }
